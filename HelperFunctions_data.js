...
export function getCustomTime() {
	const unix = parseInt(Settings_data.customTime);
...
export function convertHoursToTimeString(hours, includeSeconds = true, limitTo24Hours = true) {
	...
	let ampm = '';
	if (limitTo24Hours && !Settings_data.use24HourTime) {
		ampm = ' ' + getAmPm(h);
...
export function convertDateToShortTime(date) {
	...
	let ampm = '';
	if (!Settings_data.use24HourTime) {
		ampm = ' ' + getAmPm(h);
		h = convert24hourTo12hour(h);
	}
...
export function getHashedLocation() {
	let loc = Settings_data.activeLocation?.NAME;
	if (loc !== undefined) {
...
export function getHashedCustomTime() {
	if (Settings_data.customTime != 'now') {
		return '@' + Settings_data.customTime;
	}
	return '';
}
